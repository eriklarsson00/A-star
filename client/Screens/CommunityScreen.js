...
export const CommunityScreen= () => {
	...
	const [IncorrectCommunityPassword, setIncorrectCommunityPassword] = React.useState(false);
	const [joinPrivateCommunity, setJoinPrivateCommunity] = React.useState(false);
	const [avaliableCommunities, setAvaliableCommunities] = React.useState(['Ängsklockan', 'Rackarberget', 'Majklockan']);
	const [communityPasswordInput, setCommunityPasswordInput] = React.useState('');
	...
	{id: 5, memberAmount: 11, name: 'ITC', description: 'beskrivning', location: 'plats', imgurl: 'https://image.shutterstock.com/image-vector/vector-illustration-cool-detailed-red-260nw-94498447.jpg', private: true, password: 'psw'}, 
	...
	const CrossIcon = () => (
		<Icon style={ styles.crossStyle} fill={theme['color-basic-600']} name='close-circle-outline'/>
	);
	...
	const printExistingCommunities = ({ item, index }) => (
		<TouchableOpacity style={[styles.touchableStyle, {backgroundColor:(chosenCommunity == item.name) ? theme['color-basic-300'] : theme['color-basic-100']}]}
		onPress={()=>{(chosenCommunity == item.name) ? setChosenCommunity('') : setChosenCommunity(item.name);}}>
	  <View style={styles.communityContainer}>
		 <View style={{width: 215}}> 
		  <Text style={tw`text-lg`}>{item.name}</Text>
		  </View>{item.private && <LockIcon/>}
	  </View>
	  {chosenCommunity == item.name && <View>
		 <Divider style={{marginBottom: 5, alignSelf : 'stretch', backgroundColor : theme['color-basic-400']}}/> 
		<View style={styles.communityImageContainer}>
	  <Image style={styles.communityImage} source={{uri: item.imgurl, height:80, width:80} }/>
		...
		{item.private && <Text style={tw`font-semibold`}>Privat grannskap</Text>}
		<Text>Antal medlemmar: {item.memberAmount}</Text>
		<Text>Plats: {item.location}</Text>
		<Button style={{margin:5}} onPress={() => item.private ? setJoinPrivateCommunity(true): setJoinCommunity(true)}>Gå med</Button>
		  </View>}
	  </TouchableOpacity>);
	
	const renderIcon = (props) => { return <Icon {...props} name={props.iconName} />; };
	return (
			<Layout style={styles.container}>
				...
				accessoryLeft={(props) => renderIcon({...{iconName: 'search'}, ...props}) }
    />
				...
				<Modal
					...
					<TouchableOpacity onPress={()=> setVisible(false)}><CrossIcon/></TouchableOpacity>
					</View>
					<Divider/>
					<List style={styles.dataBaseList} data={ dataBaseCommunities.filter(comm => !avaliableCommunities.includes(comm.name))}  ItemSeparatorComponent={Divider} renderItem={printExistingCommunities} key={giveKey}/>
					<Button style={tw`mt-2`} onPress={() => setVisible(false)}>Skapa nytt grannskap</Button>
					<Divider/>
					</Card>
				</Modal>
				<Modal
					...
					<Card disabled={true}>
					<Text style={tw`text-lg font-semibold text-center`}>Vill du gå med i {chosenCommunity} ?</Text>
					<View style={{flex:1, flexDirection:'row', width: 300, justifyContent: 'center'}}>
					<Button style={{width: 100, marginRight: 15}} onPress={() => {setAvaliableCommunities([...avaliableCommunities, chosenCommunity]); setJoinCommunity(false); setVisible(false)}}>Ja</Button>
					<Button style={{width: 100, marginLeft: 15}} onPress={() => setJoinCommunity(false)}>Nej</Button>
					</View>
					<Divider/>
					</Card>
				</Modal>
				<Modal
					...
					<Card disabled={true}>
					<Text style={tw`text-lg font-semibold text-center`}>{chosenCommunity} är ett privat grannskap</Text>
					<View style={{flex:1, flexDirection:'column', width: 300, justifyContent: 'center'}}>
					{IncorrectCommunityPassword && <Text>Fel lösenord!</Text>}
					<Input label='skriv in lösenord för grannskapet för att gå med' value={communityPasswordInput} onChangeText={nextValue => setCommunityPasswordInput(nextValue)}/>
					<View style={{flex:1, flexDirection: 'row', justifyContent: 'center'}}>
					<Button style={{width: 100, margin: 15}} onPress={() => tryPassword()}>OK</Button>
					<Button style={{width: 100, margin: 15}} onPress={() => {setJoinPrivateCommunity(false); setIncorrectCommunityPassword(false)} }>Avbryt</Button>
					</View>
					</View>
					</Card>
				</Modal>
